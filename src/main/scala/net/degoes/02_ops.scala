...
object email_filter {
  ...
  final case class EmailFilter(matches: Email => Boolean) { self =>
     ...
     */
    def &&(that: EmailFilter): EmailFilter = EmailFilter { e =>
      self.matches(e) && that.matches(e)
    }
     ...
     */
    def unary_! : EmailFilter = EmailFilter { e =>
      !matches(e)
    }
  }
  object EmailFilter {
...
object contact_processing {
  ...
  final case class SchemaMapping(map: ContactsCSV => MappingResult[ContactsCSV]) { self =>
    ...
    def orElse(that: SchemaMapping): SchemaMapping = SchemaMapping { csv =>
      self.map(csv) orElse that.map(csv)
    }
    ...
    def protect(columnNames: Set[String]): SchemaMapping = SchemaMapping { csv =>
      self.map(csv.copy(schema = csv.schema.copy(columnNames = columnNames.toList)))
    }
  }
  object SchemaMapping {
    ...
    def rename(oldName: String, newName: String): SchemaMapping = SchemaMapping { csv =>
      MappingResult.fromOption(Option(csv.rename(oldName, newName)), s"Error when renaming $oldName into $newName")
    }
    ...
    def combine(leftColumn: String, rightColumn: String)(
      newName: String
    )(f: (String, String) => String): SchemaMapping = SchemaMapping { csv =>
      MappingResult
        .fromOption(csv.combine(leftColumn, rightColumn)(newName)(f), s"Can't combine $leftColumn and $rightColumn")
    }
    ...
    def relocate(column: String, j: Int): SchemaMapping = SchemaMapping { csv =>
      MappingResult.fromOption(csv.relocate(column, j), s"Error when relocating $column")
    }
    ...
    def delete(name: String): SchemaMapping = SchemaMapping { csv =>
      MappingResult.fromOption(Option(csv.delete(name)), s"Error when deleting $name")
    }
  }
  ...
  lazy val schemaMapping: SchemaMapping =
    SchemaMapping.rename("email", "email_address") +
      ...
      SchemaMapping.relocate("email", 1)
...
object education {
  ...
  lazy val exampleQuiz: Quiz =
    Quiz(Question.TrueFalse("Are cat the most cute animal?", Checker.isFalse(25))) +
      Quiz(Question.Text("2^10=?", Checker.isText(10)("1024"))).bonus.check(_.wrongPoints != 0)(
        Quiz(Question.MultipleChoice("3+3*3=?", Vector("18, 12, 6, -5"), Checker.isMultipleChoice(200)(1))).bonus,
        Quiz.empty
      )
}
